...
def check( tgzFile, MustExist, MustRemove):
    ...
    regExp = re.compile(r'^[brtd]\d{8}_hw\d\.tgz$')
    baseName = os.path.basename(tgzFile)

    if not regExp.match(baseName):
        return 1, toErrorStr("Illegal File Name \"" + baseName + "\"")
    ...
    studentID = baseName[0:9]
    fileList = [ x.strip("./") for x in out.decode().split()]
...
if __name__ == '__main__':
    ...
    if returnCode:
        ...
        MustExist  = [ x.strip() for x in open(MustExist).readlines() ]
        MustRemove = [ x.strip() for x in open(MustRemove).readlines() ]
